...
from dataloader import PennnFudanDataset
from dataloader import get_dataset_loaders, get_coloured_mask, intersection_over_union
import torch
import torchvision
...
class Pedestrian_Segmentation:
    def __init__(self):
        ...
        # Hyperparameters
        self.root = "D:/研一下/作业/计算机视觉/PennFudanPed/PennFudanPed"
    ...
    def train(self):
        ...
        gugu=mask_rcnn_transfer_learning(is_finetune=False)
        t1 = time.time()
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, targets) in enumerate(self.train_loader):
                ...
                print("Loss = ", L.item(), " batch = ", i, "/", bathes_per_epoch)
                train_loss+=L.item()*len(targets)
            train_loss/=len(self.train_loader)
            writer.add_scalar('Loss',train_loss,epoch)
            #writer.add_graph(gugu,images)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            print(f"Recall = {Recall} & Precision = {Percesion} ")
            return Recall, Percesion

    def save(self):
        torch.save(self.mask_RCNN, self.weights_path)

    def save2(self):
        torch.save(self.mask_RCNN.state_dict(), './state_dict.pth')
    ...
    def detect(self, path):
        ...
        with torch.no_grad():
            ...
            #cv2.waitKey(0)
            cv2.imwrite(path[:-4]+'-detected.png', img)
...
'''
root = "D:\Test"
paths = sorted(os.listdir("D:\Test"))
for i in paths:
